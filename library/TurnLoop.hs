...
newLobby :: (MonadIO m, Ord sessionId) => m (Lobby sessionId rep userId m)
newLobby = liftIO $ do
  p <- liftIO $ newTVarIO []
...
newRegistry :: (MonadIO m, Ord userId) => m userId -> m (Registry userId user m)
newRegistry genUserId = do
  w <- liftIO $ newTVarIO Map.empty
  return Registry
...
newResults :: (MonadIO m, Ord sessionId) => m (Results sessionId rep userId state extra m)
newResults = liftIO $ do
  w <- liftIO $ newTVarIO Map.empty
  return Results
